...
router.post('/upload', auth, uploadMiddleware, async (req, res) => {
  try {
    ...
    const uploadPromises = req.files.map(file => cloudflareR2.uploadImage(file));
    ...
    if (failedUploads.length > 0) {
      ...
      await Promise.all(
        successfulUploads.map(upload => cloudflareR2.deleteImage(upload.key))
      );
...
router.delete('/:key', auth, async (req, res) => {
  try {
    const result = await cloudflareR2.deleteImage(req.params.key);
